...
class DeepseekClient {
    constructor(model) {
        ...
        if (model === 'deepseek-chat') {
            this.apiKey = config.get('apiKeys.deepseekChat', '');
            this.baseUrl = 'https://api.deepseek.com/v1';
        }
        else {
            this.apiKey = config.get('apiKeys.deepseekV3', '');
...
class GoogleClient {
    constructor() {
        const config = vscode.workspace.getConfiguration('ai-proofread');
        this.apiKey = config.get('apiKeys.google', '');
        if (!this.apiKey) {
...
async function processJsonFileAsync(jsonInPath, jsonOutPath, options = {}) {
    // 设置默认值
    const { startCount = 1, stopCount, model = 'deepseek-chat', rpm = 15, maxConcurrent = 3, onProgress } = options;
    // 读取输入JSON文件
